...
if(!isset($_GET['function']) || $_GET['function'] == '') {
	...
	if($function == "accueil") {
        ...
        elseif (isset($_POST['renommer_ma_piece'])){
            $nom= htmlspecialchars($_POST['renommer']);
            $renom= $bdd->prepare('UPDATE mode_obj_pers SET nom_mode = :nom WHERE id= :id');
            $renom->execute(array('nom'=> $nom, 'id'=>$_POST['choix']));
        }
        $nombre = selection($bdd,'mode_obj_pers','count(*) as nbm', 'id_maison', $_SESSION['maison']);
        $req = $nombre -> fetch();
        
        $requser = selection($bdd,'mode_obj_pers','*','id_maison', $_SESSION['maison']);
        
        
            
    
    }elseif($function == 'mode_ajout'){
        $content = 'mode_ajout_c';

         if (isset($_POST['check']) AND isset($_POST['enregistrer'])){

            $temp=htmlspecialchars($_POST['temperature']);
            $lum=htmlspecialchars($_POST['lumiere']);
            $vol=htmlspecialchars($_POST['volet']);
            $cam=htmlspecialchars($_POST['camera']);

            foreach ($_POST['check'] as $element)
            {
                $values = array('nom_mode'=> $element, 'id_maison'=>$_SESSION['maison'], 'temperature'=> $temp, 'lumiere'=> $lum, 'volet'=> $vol, 'camera'=> $cam);
                $table ='mode_obj_pers';
                $stm = insertion($bdd,$values,$table);
            }
        }
...
//Modification du mode
    }elseif($function == 'mode_renommer'){
        $content = 'mode_renommer_c';


               if (isset($_POST['renommer_mode'])){
