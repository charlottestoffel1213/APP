...
<input type="submit" name="submit" value="MODE NUIT">
</form>
</html>
...
/*if(isset($_SESSION['id']))*/
		...
		{
			$tempN = $bdd->query('SELECT temperature FROM mode_obj WHERE id = 1 ');
			$requete1 = $tempN -> fetch();
			echo $requete1[0] . ' et ' ;
      		
      		// $req1 = value();

			$lumN = $bdd->query('SELECT luminosite FROM mode_obj WHERE id=1');
			$requete2 = $lumN->fetch();
			echo $requete2[0] . ' et ';

			$secuN = $bdd->query('SELECT securite FROM mode_obj WHERE id=1');
			...
			// Boucle permettant de modifier toutes les valeurs demandées
